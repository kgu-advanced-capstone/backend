...
public class ResumeApiService {
    ...
    public void generate(String email) {
        ...
        Resume resume = resumeService.findByUserId(userId).orElseGet(() -> Resume.createFor(userId));
        ...
        Map<Long, Project> projectMap = projectService.getAllByIds(projectIds).stream().collect(Collectors.toMap(Project::getId, p -> p));
        Map<Long, Experience> experienceMap = experienceService.findByProjectIdsAndUserId(projectIds, userId);

        List<ResumedExperience> experiences = memberships.stream().filter(member -> projectMap.containsKey(member.getProjectId())).filter(member -> projectMap.get(member.getProjectId()).getStatus() != ProjectStatus.RECRUITING).map(member -> {
            ...
            if (experience != null && experience.getAiSummary() != null && !experience.getAiSummary().isBlank()) {
                ...
                keyPoints = experienceSummarizer.generateKeyPoints(project.getTitle(), project.getDescription(), project.getCategory(), project.getSkills(), experienceContent);
                Experience expToUpdate = experience != null ? experience : Experience.create(new ExperienceCreateCommand(userId, project.getId(), ""));
